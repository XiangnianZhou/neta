...
Deno.test('assParser: input Uint8Array', (): void => {
    const data = Deno.readFileSync('./test.ass');
    const actual2 = assParser(data);
    assertEquals(actual.events, actual2.events);
    assertEquals(actual.scriptInfo, actual2.scriptInfo);
    assertEquals(actual.v4Styles, actual2.v4Styles);
});
...
Deno.test('convert ass to srt', (): void => {
    ...
    assertEquals(assParser(assString).toSrt(), expectedSrt);
});
...
Deno.test('convert ass to text', (): void => {
    ...
    assertEquals(assParser(assString).toText(), expectedText);
});
